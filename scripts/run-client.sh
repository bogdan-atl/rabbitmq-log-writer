...
REDIS_DEAD_LETTER_KEY="${REDIS_DEAD_LETTER_KEY:-udp-logger:queue:dead-letter}"
REDIS_MAX_RETRIES="${REDIS_MAX_RETRIES:-10}"
REDIS_VISIBILITY_TIMEOUT="${REDIS_VISIBILITY_TIMEOUT:-30s}"
REDIS_REAPER_INTERVAL="${REDIS_REAPER_INTERVAL:-5s}"
...
if [ "$QUEUE_BACKEND" = "redis" ]; then
    echo "Redis: $REDIS_ADDR db=$REDIS_DB key=$REDIS_QUEUE_KEY retries=$REDIS_MAX_RETRIES"
else
...
export REDIS_DEAD_LETTER_KEY
export REDIS_MAX_RETRIES
export REDIS_VISIBILITY_TIMEOUT
export REDIS_REAPER_INTERVAL
